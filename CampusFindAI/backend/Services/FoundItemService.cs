...
public class FoundItemService(IFoundItemRepository repository, IImageRepository imageRepository, IReportImageStorage imageStorage, IMatchService matchService, IMatchRepository matchRepository, IClaimRepository claimRepository, IReferenceDataService referenceDataService, IAuditLogService auditLogService) : IFoundItemService
{
    public async Task<FoundItemDto> CreateAsync(string userId, CreateFoundItemDto request, CancellationToken ct = default)
    {
        ...
        await matchService.RefreshForFoundItemAsync(item.Id, ct);
    ...
    public Task<IReadOnlyList<VerificationQuestionDto>> GetOwnershipVerificationQuestionsAsync(CancellationToken ct = default) => Task.FromResult((IReadOnlyList<VerificationQuestionDto>)OwnershipVerificationQuestions.All.Select(question => new VerificationQuestionDto { Id = question.Id, Question = question.Question, Type = question.Type }).ToList());
    public async Task<FounderVerificationResponseDto> GetFounderVerificationAsync(string userId, Guid id, CancellationToken ct = default) => MapFounderVerification(await GetOwnedAsync(userId, id, ct));
    public async Task<FounderVerificationResponseDto> SaveFounderVerificationAsync(string userId, Guid id, SaveFounderVerificationAnswersDto request, CancellationToken ct = default)
    {
        var item = await GetOwnedAsync(userId, id, ct);
        await EnsureNoActiveClaimAsync(id, ct);
        item.FounderVerificationAnswersJson = OwnershipVerificationQuestions.SerializeFounderAnswers(request.Answers);
        await repository.UpdateFounderVerificationAnswersAsync(id, item.FounderVerificationAnswersJson, ct);
    ...
    private static FounderVerificationResponseDto MapFounderVerification(FoundItem item)
    {
        var answers = OwnershipVerificationQuestions.ReadFounderAnswers(item.FounderVerificationAnswersJson);
        return new FounderVerificationResponseDto
        {
            FoundItemId = item.Id,
            IsComplete = answers.Count == OwnershipVerificationQuestions.All.Count,
            Status = answers.Count == OwnershipVerificationQuestions.All.Count ? "Ready" : "Incomplete",
            Questions = OwnershipVerificationQuestions.All.Select(question => new FounderVerificationQuestionDto
            {
                Id = question.Id,
                Question = question.Question,
                Type = question.Type,
                Answer = answers.FirstOrDefault(answer => answer.QuestionId == question.Id)?.Answer ?? string.Empty
            }).ToList()
        };
    }
