    ...
    private async Task PersistNewSuggestionsAsync(IReadOnlyList<MatchDto> suggestions, CancellationToken cancellationToken)
    {
        foreach (var suggestion in suggestions)
        {
            ...
            if (suggestion.ConfidenceScore >= NotificationThreshold)
                await notificationService.CreateAsync(suggestion.LostItemUserId, $"Possible match found: Your {suggestion.LostItemTitle} has a {Math.Round(suggestion.ConfidenceScore)}% match with a recently found item.", "/my-matches", "MATCH_FOUND", cancellationToken);
        }
