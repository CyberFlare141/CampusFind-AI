...
public sealed class StructuredCampusLocationTests
{
    ...
    public async Task FoundItemCreation_AcceptsNaturalLocationWithoutStructuredIds()
    {
        ...
        {
            Title = "Black wallet",
            FounderVerificationAnswers = ["Small scratch on the inside.", "Blue keychain attached.", "Initials written under the flap."],
    ...
    private static CreateFoundItemDto RequestFound(CampusSeed seed) => new() { Title = "Wallet", FounderVerificationAnswers = ["Small identifying scratch.", "Blue keychain attached.", "Initials written under the flap."], BuildingId = seed.BlockA.Id, FloorId = seed.AFloor1.Id, LocationId = seed.AFloor1Location.Id, LocationDetails = "Near the lift" };
