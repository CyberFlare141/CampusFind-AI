...
export default function FoundItemFormPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate(`/found-items/${created.id}`, { replace: true, state: { justCreated: true } });
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="form-field">
            ...
            <input
              id="title"
              type="text"
              placeholder="e.g. Silver iPhone with cracked case"
              value={title}
          ...
          <div className="form-field">
            ...
            <textarea
              id="description"
              placeholder="Color, brand, condition, where exactly you found it, etc."
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              rows={4}
            />
            <span className="hint">Avoid listing highly identifying details publicly &mdash; save those to verify a claimant.</span>
          </div>

          <div className="form-field">
            <label htmlFor="foundAt">When did you find it?</label>
            <input
              id="foundAt"
              type="datetime-local"
              value={foundAt}
              ...
              max={latestDate}
            />
            {fieldErrors.foundAt && <span className="field-error">{fieldErrors.foundAt}</span>}
