  ...
  return (
    <article className="match-comparison-card">
      ...
      <div className="match-card-footer">
        <div className="match-rationale"><p>{match.explanation}</p>{match.matchedAttributes?.length > 0 && <ul>{match.matchedAttributes.map(attribute => <li key={attribute}>{attribute}</li>)}</ul>}<ConfidenceBar score={match.confidenceScore} /></div>
        <Link className="btn btn-primary" to={'/found-items/' + match.foundItemId}>View found item</Link>
      </div>
    </article>
  );
}
...
export default function MyMatchesPage() {
  ...
  useEffect(() => { getMyMatches().then(setMatches).catch(err => setError(err.message)).finally(() => setLoading(false)); }, []);
  ...
  return <div className="page-container-wide matches-page">
    ...
    {!error && matches.length === 0 ? <EmptyState title="No suggested matches yet" message="When a compatible found-item report is submitted, it will appear here automatically." action={<Link className="btn btn-primary" to="/found-items">Browse found items</Link>} /> : <div className="match-comparison-list">{matches.map(match => <MatchItem key={match.id} match={match} />)}</div>}
  </div>;
}
